...
public class RUHungry {
	...
	// WRITE YOUR CODE HERE
        int length = StdIn.readInt(); // 1 
        StdIn.readLine(); // 
        categoryVar = new String[length]; // type of food
        menuVar = new MenuNode[length]; // nodes of items that correspond to category 
        int[] arrOfIngredients;
        MenuNode head = null;
        
        //What category
        for(int i = 0; i < length; i++){
            String category = StdIn.readLine(); // 2 
            categoryVar[i] = category; 
            int count = StdIn.readInt(); // 3 
            StdIn.readLine(); // 
            // dishes in said category 
            for(int j = 0; j<count; j ++){
                  
                String dishName = StdIn.readLine();  // 4 
                //Ingredients ID
                
                int numOfIngredients = StdIn.readInt();  // 5 
                arrOfIngredients = new int[numOfIngredients]; // 6 
                for(int k = 0; k< numOfIngredients; k++){
                    arrOfIngredients[k] = StdIn.readInt();
                }
                StdIn.readLine();
                Dish newDish = new Dish(category, dishName, arrOfIngredients);

                MenuNode newItem = new MenuNode(newDish, head);
                head = newItem; 
            }
             menuVar[i] = head;
             head = null;
	...
	// WRITE YOUR CODE HERE
        int currentId = newNode.getIngredient().getID(); 
        int index = currentId % stockVarSize;
	...
	// WRITE YOUR CODE HERE
        ...
        if(ingredientID == -1){
            ...
            target.getIngredient().updateStockLevel(stockAmountToAdd);
        }
	...
	// WRITE YOUR CODE HERE
       ...
       // double profit = 0;  
           ...
           int[] dishArray = currentDish.getStockID();
           for(int j = 0; j<dishArray.length; j++){
            StockNode currentStock = findStockNode(dishArray[j]);
            cost = cost + currentStock.getIngredient().getCost();
           }
           price =  cost * 1.2;
           currentDish.setPriceOfDish(price);
           currentDish.setProfit(price - cost);
           ptr = ptr.getNextMenuNode();
           }
	...
	// WRITE YOUR CODE HERE
        int stockSize = StdIn.readInt(); 
        stockVarSize = stockSize; 
        stockVar = new StockNode[stockSize];
       // StdIn.readLine();
        while(StdIn.readLine()!=null){
            int ingredID = StdIn.readInt();
            StdIn.readChar();
            String name  =  StdIn.readLine();
            double cost =  StdIn.readDouble();
           // StdIn.readLine();
            int numInStock = StdIn.readInt(); 
           // StdIn.readLine();
            Ingredient newIngredient = new Ingredient(ingredID, name, numInStock, cost); 
            StockNode newStock = new StockNode(newIngredient, null);
	...
	// WRITE YOUR CODE HERE
        // transactionVar = head
        TransactionNode currentNode = new TransactionNode(data, null);
        
        if(transactionVar == null){
            transactionVar = currentNode; 
        } else{
	...
	// WRITE YOUR CODE HERE
        ...
        MenuNode currentMenu = findDish(dishName);
        Dish currentDish = currentMenu.getDish();
        int[] ingredientArray =  currentDish.getStockID();
        for(int i = 0; i < ingredientArray.length; i++){
            StockNode currentStock = findStockNode(ingredientArray[i]);
           Ingredient currentIngredient =  currentStock.getIngredient();
           if(currentIngredient.getStockLevel() < numberOfDishes){
            isAvailable = false; 
           }
        } 
        return isAvailable; // update the return value
    }
	...
	// WRITE YOUR CODE HERE
        ...
        boolean isAvailable = checkDishAvailability(dishName, quantity);
        if(isAvailable == true){
            TransactionData newData = new TransactionData();
            newData.setType("order");
            newData.setItem(dishName);
            newData.setAmount(quantity);
            double profit = findDish(dishName).getDish().getProfit() * quantity; 
            newData.setProfit(profit);
            newData.setSuccess(true);
            addTransactionNode(newData);
            
            
            // update stock 
            MenuNode currentMenu = findDish(dishName);
            int[] currentStockArray =  currentMenu.getDish().getStockID();
            for(int i = 0; i < currentStockArray.length; i++){
                StockNode currentStock = findStockNode(currentStockArray[i]);
               // currentStock.getIngredient().get
                int currentNum = currentStock.getIngredient().getStockLevel(); 
                int newPrice = currentNum - quantity;
                currentStock.getIngredient().setStockLevel(newPrice);
            } 
        }else{
            TransactionData newData = new TransactionData();
            newData.setType("order");
            newData.setItem(dishName);
            newData.setAmount(quantity);
            newData.setProfit(0.0);
            newData.setSuccess(false);
            addTransactionNode(newData);
            MenuNode curMenuNode = findDish(dishName); 

            String cat = curMenuNode.getDish().getCategory();
            MenuNode ptr = null;
            MenuNode newPtr = null;
            MenuNode categoryNode = null; 
            MenuNode curMenunodePtr = curMenuNode; 
            boolean atEnd = false; 
            
            // siimulated ordering next in menu 
            for(int i = 0; i<menuVar.length; i++){
                if(menuVar[i].getDish().getCategory().equals(cat)){
                    //traverse from node after current node to end of list 
                    curMenuNode = curMenuNode.getNextMenuNode();
                    while(curMenuNode != null){
                        if(checkDishAvailability(curMenuNode.getDish().getName(), quantity)){
                            TransactionData newDi = new TransactionData("order", curMenuNode.getDish().getName(), quantity, findDish(curMenuNode.getDish().getName()).getDish().getProfit() * quantity, true);
                            addTransactionNode(newDi);
                            //update stock 
                            int[] upStock = curMenuNode.getDish().getStockID();
                            for(int j = 0; j<upStock.length; j++){
                                StockNode currStock = findStockNode(upStock[j]);
                                int currentPr = currStock.getIngredient().getStockLevel();
                                int newPr = currentPr - quantity; 
                                currStock.getIngredient().setStockLevel(newPr);
                            }
                            break; 
                        } else{
                            TransactionData newDis = new TransactionData("order", curMenuNode.getDish().getName(), quantity, 0, false); // works for rn but this line is the error
                            addTransactionNode(newDis);
                        }
                        curMenuNode = curMenuNode.getNextMenuNode();
                    }
                    MenuNode beginning = menuVar[i];
                    while(beginning != curMenunodePtr){
                        if(checkDishAvailability(beginning.getDish().getName(), quantity)){
                            TransactionData newDish = new TransactionData("order", beginning.getDish().getName(), quantity, findDish(beginning.getDish().getName()).getDish().getProfit() * quantity, true);
                            addTransactionNode(newDish);
                            //update stock 
                            int[] upStock = beginning.getDish().getStockID();
                            for(int j = 0; j<upStock.length; j++){
                                StockNode currStock = findStockNode(upStock[j]);
                                int currentPr = currStock.getIngredient().getStockLevel();
                                int newPr = currentPr - quantity; 
                            ...
                            TransactionData newDishs = new TransactionData("order", beginning.getDish().getName(), quantity, 0, false); // works for rn but this line is the error
                            addTransactionNode(newDishs);
                        }
                        beginning = beginning.getNextMenuNode();
                    }
                }
            }
              ...
              /*for(int i = 0; i< menuVar.length; i++){
                ...
                if(menuVar[i].getDish().getCategory().equals(cat)){
                    //current to end 
                     ...
                     while(curMenunodePtr!= null ){
                        if(checkDishAvailability(curMenuNode.getDish().getName(), quantity)){
                            TransactionData newDi = new TransactionData("order", curMenuNode.getDish().getName(), quantity, findDish(curMenuNode.getDish().getName()).getDish().getProfit() * quantity, true);
                            addTransactionNode(newDi);
                            //update stock 
                            int[] upStock = curMenunodePtr.getDish().getStockID();
                            for(int j = 0; j<upStock.length; j++){
                                StockNode currStock = findStockNode(upStock[j]);
                                int currentPr = currStock.getIngredient().getStockLevel();
                                int newPr = currentPr - quantity; 
                            ...
                            break;
                        } else if(!curMenuNode.getDish().getName().equals(curMenunodePtr.getDish().getName())){
                            TransactionData newDis = new TransactionData("order", curMenuNode.getDish().getName(), quantity, 0, false); // works for rn but this line is the error
                           // addTransactionNode(newDis);
                        }
                        curMenunodePtr = curMenunodePtr.getNextMenuNode(); 

                    }

                    //checks begining to current 
                    MenuNode begPtr = menuVar[i];
                    // FIX THIS DO NOT WORK PLEASE HELP 
                    // try to fix the go through loop 
                    while(begPtr != curMenuNode ){
                        if(checkDishAvailability(begPtr.getDish().getName(), quantity)){
                            TransactionData newDish = new TransactionData();
                            newDish.setType("order");
                            newDish.setItem(begPtr.getDish().getName());
                            newDish.setAmount(quantity);
                            double profit = findDish(begPtr.getDish().getName()).getDish().getProfit() * quantity; 
                            newDish.setProfit(profit);
                            newDish.setSuccess(true);
                            addTransactionNode(newDish);
                            //update stock 
                            int[] upStock = begPtr.getDish().getStockID();
                            for(int j = 0; j<upStock.length; j++){
                                StockNode currStock = findStockNode(upStock[j]);
                                int currentPr = currStock.getIngredient().getStockLevel();
                                int newPr = currentPr - quantity; 
                            ...
                            break; 
                            // error i in the part where tarting from begging 
                        } else{
                            TransactionData newDi = new TransactionData();
                            newDi.setType("order");
                            newDi.setItem(begPtr.getDish().getName());
                            newDi.setAmount(quantity);
                            double profit = findDish(begPtr.getDish().getName()).getDish().getProfit() * quantity; 
                            newDi.setProfit(profit);
                            newDi.setSuccess(false);
                            addTransactionNode(newDi);
                        }
                        begPtr = begPtr.getNextMenuNode();
                    }
	...
	// WRITE YOUR CODE HERE
        ...
        TransactionNode ptr = transactionVar; 
        while(ptr != null){
            totalProfit = totalProfit + ptr.getData().getProfit();
            ptr = ptr.getNext(); 
        }
	...
	// WRITE YOUR CODE HERE
        ...
        double totalProft = profit();
        //boolean isAva = checkDishAvailability(ingredientName, quantity); 
        StockNode currentIngredient = findStockNode(ingredientName);
        int stock = currentIngredient.getIngredient().getStockLevel(); 

         if(totalProft >  50.00 && (stock >= quantity)){
            TransactionData newDono = new TransactionData("donation", ingredientName, quantity, 0, true);
            addTransactionNode(newDono);
            updateStock(ingredientName, -1, -quantity);
           
        } else{
            TransactionData newbadDone = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(newbadDone);
        }
    }
	...
	// WRITE YOUR CODE HERE
        StockNode currentIngredient = findStockNode(ingredientName);
        int stock = currentIngredient.getIngredient().getStockLevel(); 
        double restockPrice = currentIngredient.getIngredient().getCost() * quantity; 
        double totalProfit = profit(); 

        if(totalProfit >    restockPrice){
            
            TransactionData newRestock = new TransactionData("restock", ingredientName, quantity, -restockPrice, true);
            addTransactionNode(newRestock);
            //update stock 
            updateStock(ingredientName, -1, quantity);
        } else{
            TransactionData newRestock1 = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(newRestock1);
        }
    }
